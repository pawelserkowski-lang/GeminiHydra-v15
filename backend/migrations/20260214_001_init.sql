...
CREATE INDEX idx_gh_chat_created ON gh_chat_messages (created_at DESC);

-- gh_memories
CREATE TABLE gh_memories (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    agent TEXT NOT NULL,
    content TEXT NOT NULL,
...
CREATE INDEX idx_gh_mem_agent ON gh_memories (agent);
CREATE INDEX idx_gh_mem_importance ON gh_memories (importance DESC);

-- gh_knowledge_nodes + gh_knowledge_edges
CREATE TABLE gh_knowledge_nodes (
    id TEXT PRIMARY KEY,
    node_type TEXT NOT NULL,
    label TEXT NOT NULL
);
CREATE TABLE gh_knowledge_edges (
